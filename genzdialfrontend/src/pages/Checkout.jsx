...
import CheckoutOtpLogin from '../components/CheckoutOtpLogin.jsx';
...
export default function Checkout() {
    ...
    // Pincode auto-fetch (city/state/area) state
    const [pincodeStatus, setPincodeStatus] = useState({ loading: false, error: '', areas: [] });
    ...
    useEffect(() => {
        ...
        if (pin.length !== 6) {
            setPincodeStatus({ loading: false, error: '', areas: [] });
            return;
        }
        let cancelled = false;
        setPincodeStatus({ loading: true, error: '', areas: [] });
            ...
            try {
                ...
                if (result?.Status === 'Success' && result.PostOffice?.length) {
                    ...
                    setAddress((prev) => ({
                        ...
                        area: offices.length === 1 ? offices[0].Name : prev.area,
                    }));
                    setPincodeStatus({
                        loading: false,
                        error: '',
                        areas: offices.length > 1 ? offices.map((o) => o.Name) : [],
                    });
                } else {
                    setPincodeStatus({ loading: false, error: 'Pincode not found, please enter city/state manually', areas: [] });
                }
            } catch {
                if (!cancelled) {
                    setPincodeStatus({ loading: false, error: 'Could not fetch area right now, enter manually', areas: [] });
                }
            }
        }, 400);
    ...
    return (
        <section className="container section">
            ...
            <div className="checkout-steps">
                <span className={step === 'address' ? 'active' : (step === 'auth' || step === 'pay' || step === 'done' ? 'done' : '')}>1. Address</span>
                <span className={step === 'auth' ? 'active' : (step === 'pay' || step === 'done' ? 'done' : '')}>2. Login</span>
                <span className={step === 'pay' ? 'active' : (step === 'done' ? 'done' : '')}>3. Payment</span>
                <span className={step === 'done' ? 'active' : ''}>4. Confirm</span>
            </div>
                        ...
                        <form className="address-form" onSubmit={submitAddress}>
                            ...
                            <div className="form-row">
                                <input placeholder="Pincode*" inputMode="numeric" maxLength={6} value={address.pincode}
                                    onChange={(e) => setAddress({ ...address, pincode: e.target.value.replace(/\D/g, ''), area: '' })} />
                                <input placeholder="Country" value={address.country} readOnly />
                            </div>
                            {pincodeStatus.loading && (
                                <div style={{ fontSize: '12px', color: '#565959', marginTop: '-8px', marginBottom: '10px' }}>Fetching area...</div>
                            )}
                            {pincodeStatus.error && (
                                <div style={{ fontSize: '12px', color: '#d9534f', marginTop: '-8px', marginBottom: '10px' }}>{pincodeStatus.error}</div>
                            )}
                            {pincodeStatus.areas.length > 0 && (
                                <select
                                    value={address.area}
                                    onChange={(e) => setAddress({ ...address, area: e.target.value })}
                                    style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #888c8c', marginBottom: '10px' }}
                                >
                                    <option value="">Select Area / Locality*</option>
                                    {pincodeStatus.areas.map((a) => (
